...
instruction_s decode_opcode() {
    if (cpu.opcode != 0xCB) {
        return instruction_table[cpu.opcode];
    } else {
        cpu.opcode = fetch_byte();
        return cb_instruction_table[cpu.opcode];
    }
}
...
const instruction_s instruction_table[256] = {
    ...
    [0x0f] = {"RRCA", 1, 1, rrca},
    [0x10] = {"STOP", 1, 1, stop}, // todo: also sometimes 2 byte length
